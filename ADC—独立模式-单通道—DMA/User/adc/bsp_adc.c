...
static void ADC_GPIO_Mode_Config(void)
{
  ...
  /* H743XIH6的ADC3_CH1使用的是PC3_C引脚，与PC3是不同的引脚，通过一个模拟开关连接，使用时需要切换 */
  /* PC3_C ------> ADC3_INP1  */
...
static void ADC_Mode_Config(void)
{
	...
	HAL_ADC_Start_DMA(&ADC_Handle, (uint32_t*)&ADC_ConvertedValue, 1);//此函数会开启DMA中断，需要用户自行配置DMA中断优先级
}
/**
  * @brief  DMA中断优先级配置函数
  * @param  无
...
{
	HAL_NVIC_SetPriority(Rheostat_ADC1_DMA_IRQ, 0, 0);
